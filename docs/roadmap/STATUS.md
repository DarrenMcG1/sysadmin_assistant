...
> **The entry named one writer and the box had three; the fix is a
> deletion because the announcement already had an owner.**
> `SNAG-AGENT-010` is **FIXED**. Three modules wrote an identical `info`
> alert row saying a weekly review was ready —
> `files/review.py:553`, `monitor/log_review.py:575` and
> `monitor/health_review.py:1055`. The entry named the first, because
> the symptom on the box was one row: only the disk review's scheduler
> path has ever fired. All three writes are gone.
>
> **The candidate the entry listed first would not have closed the
> observed row.** "Resolve the previous notice when the next review is
> generated" anchors the lifecycle to the next run; the row was written
> 2026-08-17 05:45 and the next generation was due **08-24 05:45, when
> the daemon was down** — the first `agent_runs` row that day is 07:00.
> Under that fix the row is open today at 271 h with the same wrong
> sentence. A lifecycle bounded by the thing that already failed.
>
> **The second candidate was right and its framing was not.** It
> proposed *"let the tray read `/api/files/review`"* — a tray feature
> that does not exist and did not need to. `sysadmin_tray/` contains no
> reference to any review endpoint; the consumer that mattered is
> `briefing/data.py`, whose `_gather` reads `disk_reviews`,
> `log_reviews` **and** `health_reviews` directly and always has.
>
> **The clinching observation is one payload disagreeing with itself.**
> The briefing expires a review older than `_REVIEW_FRESH_DAYS` (8).
> Measured live, `GET /api/sysadmin/briefing/preview` rendered
> Infrastructure, Overnight Logs, Filesystem, Weekly Log Review and
> Weekly System Health Review — **no Weekly Disk Review, dropped as
> stale at 11 days** — while the same envelope's alert digest carried
> `Weekly disk review ready`. The briefing declined to show the review
> and went on announcing it, one owner of the two holding a freshness
> rule.
>
> **A notice was being counted as a fault by this repository's own
> review.** `health_review._gather_alerts` takes **no severity filter**
> and `build_review_prompt` phrases the figure as *"Distinct faults
> alerted"* — so each week's three notices would be narrated as three
> faults, and the health review's own notice lands in the next health
> review's `new_titles`. The fixture had already normalised it:
> `tests/test_health_review.py` used `"new_titles": ["Weekly disk review
> ready"]` as its example of a fault. Filtering the query was refused —
> it leaves three immortal rows standing and teaches the count to ignore
> a severity `known_noise` and `COVERED_SIGNATURES` use legitimately.
>
> **Neither speaker could ever have said it.** `tray.notify_min_severity`
> and `notifications.desktop.min_severity` both read `warning`; the row
> is `info`. Nothing that was being delivered is lost. Nothing consumed
> the payload either — `details['review_id']` and `details['endpoint']`
> were written by three modules and read by none, `SNAG-CFG-001`'s shape
> three times over.
>
> **The suite was pinning the immortality as intended behaviour**, which
> is why this was found by counting the table rather than by a red test.
> `test_the_announcement_escapes_the_sysadmin_resolve_sweep` asserted the
> title matched no `RESOLVABLE_TITLE_PATTERNS` entry, reasoning that an
> announcement filed under `sysadmin` was *"one wrong suffix away from
> being resolved by the next health check"* — escaping the sweep read as
> the property to protect. Read the other way it is the defect, since
> retention deletes an `alerts` row only when it is resolved. It and its
> `chk_alert_agent` sibling retire with the write.
>
> **The guard is two halves that fail apart, and the separation was
> falsified rather than asserted.** Behaviour (the scheduler path adds
> nothing) and provenance (no `Alert(...)` built in `run_weekly_review`).
> Driven at a stand-in that **builds the row and never adds it**, only
> the provenance half fires. It is an `ast` walk rather than a grep
> because `health_review` legitimately imports `Alert` for four
> alert-delta queries — a name in an import list is not a caller.
>
> **Verified live through the real commit path**, which the `MagicMock`
> fixtures cannot reach: all three entry points driven against the real
> database with `generate_review` stubbed and the real
> `get_scheduler_session` — `alerts` at **665,937 before and after,
> delta 0 for each**, **0 rows of residue**. The standing row was
> resolved by hand, nothing in the code being able to reach it once the
> writer was gone, which also makes it retention-reachable for the first
> time: it purges around 2026-09-16.
>
> **A restart was owed and taken.** The daemon was restarted at
> **2026-08-28 13:41:23** <!--check:deploy--> <!--check:daemon_start-->
> (PID 3684053 → 3716817),
> `/health` answers **200** <!--check:health--> within 2 s, `alembic
> current` reads 017 at the packaged head <!--check:schema-->, and
> `alerts` now holds **0** unresolved rows <!--check:alerts--> with
> **0** named here <!--check:open_titles-->. The schema was checked at
> head before the signal, `SNAG-DB-005`'s rule. The fix is inert without
> the restart and the next firing is Monday 05:00.
>
> **Alembic head is 017**<!--check:migration_head--> and the live schema
> matches it. Unmoved — this sitting wrote code and no migration.
>
> **The suite is 2772**, from 2764: **9 added and 1 retired**.
> `tests/test_review_announcements.py` is the nine, over all three review
> modules rather than the one the entry named. Every one falsified: the
> alert write restored breaks both halves for that module, and a
> build-without-add stand-in breaks only provenance — which is what
> justifies keeping two halves rather than one. The three reason-tests
> were falsified at their own subjects: retention's `resolved = TRUE`,
> the two severity gates in `config.yaml`, and a `_gather_review` line
> removed from the briefing.
>
> **The entry counts were re-derived, not carried**: **99 entries either
> side, open 23 → 22**, through `estate.snags.read_snags` at
> estate-manager's committed `c545fa8` on a clean tree. The first drive
> reported `unrecognised` / 0 entries and was **one signature-read away
> from a cross-repo message accusing them of breaking the parse** —
> `read_snags` takes the document *text* and reads a path as a one-line
> document. It has taken text at every commit that has ever touched it,
> so there was no change and nothing to file. The trap is already in
> memory, warning about this exact near-miss; a correction is a claim
> too.
