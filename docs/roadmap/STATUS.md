...
> 2302052 → 2707546, and **verified live and untriggered**: the first
> `estate_judge` run afterwards, at 16:38:20, reports `surfaces_read` of
> **six** with `by_surface` carrying `hook_wiring: 0` — zero because this
> box's `settings.json` parses, not because nothing looked.
