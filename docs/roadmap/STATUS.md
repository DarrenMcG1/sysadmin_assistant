...
> **No deploy was owed, and one was taken anyway — the check's own
> stated false positive, materialised.**
> <!--check:deploy--> `sysadmin` was restarted at **2026-08-28 09:30:18**
> <!--check:daemon_start--> — by Session 107, which touched only
> `sysadmin/snag_claims.py`, a module reached from the
> `sysadmin-check-snags` console script and from **nothing in
> `create_app()`**. The deploy check compares the newest `.py` on disk
> against the daemon's start, and its docstring already prices this:
> *"a file the daemon never imports reports a restart owed, and that
> fails in the direction that costs a needless `kill -TERM`"*. The
> alternative was a red claim every preflight until an unrelated sitting
> restarted, which is worse than the two seconds. `/health` 200, PID
> 3320978 → 3532450, no `sudo`.
>
...
> being one constant, one parse and their guards.
>
> **`SNAG-PORT-001` is closed, and widening the band raises nothing.**
> The entry deferred itself on the grounds that *"widening the band
> changes what an alert family raises"*. Driven through
> `ServiceDiscoveryAgent._check_ports`' own code path against the real
> `ss`, the real `services.yaml` and the real registry document, at the
> old band and the new one: **findings 0 → 0, collisions 0 → 0, advice
> 0 → 0.** The only observable difference is `unit_audited_ports`, 12
> units → 13. The deferral was still right — the only way to know it is
> zero is to drive it, and a constant changed without the drive is a
> guess that happened to be correct.
>
> **The entry's account of its own blast radius was wrong, and both sides
> of the copy made the same mistake.** It names four comparisons —
> `wrong_unit`, `port_shared`, `duplicate_claim`, `wrong_project`.
> `in_range` has **two** production call sites and one of them is not a
> comparison: `judge_ports`' `wrong_project` gate, and
> `PortReport.unit_ports(audited_only=True)`. estate-manager had already
> caught and corrected the identical overstatement on their side the same
> day (their ADR-0054 §5 → their `SNAG-ESTATE-070`). Reading the entry
> finds the ranges; grepping the callers ranks them.
>
> **1883 could never have contributed, which is the durable half.** Its
> socket is root-owned, so `ss -p` names no holder, so it is
> `unattributed` and compared against nothing by construction —
> `collation.py`'s rule 1, this module's founding posture. Widening a
> band over a port nobody can attribute buys jurisdiction and no
> observation, which is exactly the distinction `ports_checked` exists to
> keep visible.
>
> **The drive found a five-month-old parse underneath it.**
> `_unit_from_cgroup` took the cgroup path from its **last** colon.
> `cgroup(5)` is `hierarchy:controllers:path` and only the first two
> fields are colon-free, so a D-Bus activated unit —
...
> 694 processes. `SNAG-PORT-002`, found and fixed the same sitting. What
> made it visible was the widening — 1716 is the first mis-parsed
> listener ever to fall inside an audited band — and what hid it was the
> fixtures: every existing cgroup test uses a hand-written path and
> nobody had typed one with a colon in it.
>
> **The band now has three statements and all three are pinned.**
> `PortCheckConfig.audited_ranges` is production;
> `ports.DEFAULT_AUDITED_RANGES` is the pure module's fallback, which may
> not read `config.yaml` and so cannot derive it; and the tests compare
> both against estate-manager's — now against **both** of theirs, their
> shipped `audit.yaml` as well as their code default, because leaning on
> their own pin test is the cross-repository lookup this copy exists to
> avoid.
>
> **The box, as measured at the close of this sitting.** `/health`
> answers **200** <!--check:health-->, `alembic current` reads 017 at the
> packaged head <!--check:schema-->, and `alerts` holds **1** unresolved
> row <!--check:alerts-->. What is open is `info: Weekly disk review
> ready` <!--check:open_titles--> — unmoved across this sitting, which is
> the quiet outcome and is stated because a *fall* nobody wrote about is
> the signal `SNAG-ESTATE-008` exists for.
>
> **The suite is green again and the arithmetic is stated.** 2732 → 2737,
> the five being four cgroup guards and one self-pin; the two that were
> red on a clean tree when this sitting opened are the two that closed.
> A green suite cannot witness tests that no longer exist, so the sum is
> written rather than the total alone.
---
