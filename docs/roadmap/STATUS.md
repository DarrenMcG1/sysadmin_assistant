...
> **A prediction the block makes now carries the zone it was copied
> from.** `SNAG-ESTATE-013` is **FIXED**. `ops_claims`' `check:expires`
> marker took a bare wall clock, so the one marker ever written — copied
> off an estate surface publishing `2026-08-25T03:32:17.538288+00:00` —
...
> `journal.since_timestamp`'s posture.
>
> **The format was the smaller half.** Rule 9's pin renders the marker's
> instant **into this box's zone** before looking for it in the prose.
> Naive, the entry's own block satisfied that pin — the marker said
> `03:32`, the sentence said 03:32, and both were an hour out, because
> two statements of one fact had nothing to disagree *about*. With an
> offset they visibly disagree and the note names which of them is in
> which clock. That is what makes this more than a format change, and it
> is the half the entry itself identified.
>
> **`@<epoch>` was refused, though the entry names it and
> `since_timestamp` renders exactly that for this fault.** The
> difference is the reader, not the instant: journalctl's zone is the
> reader's and unknown and its `--since` has no offset syntax at all, so
> an epoch is the only unambiguous thing it takes; here the author is a
> human who must also write the instant's wall clock into the sentence
> beside it. An epoch is unambiguous and unreadable, so accepting one
> would buy rule 8 by deleting rule 9.
>
> **The entry's "two hours" was two mechanisms and only one of them the
> marker's** — the timer fired at 04:32 local (the offset) and the
> hourly judge swept at 05:32 (the poll interval). Driven at the entry's
> own stamp through the real `check_expiry` before the fix, the
> displacement is **1 hour**, exactly this box's offset. The guard ships
> with an **empty live population and a full historical one**:
> `git log -S 'check:expires'` finds one marker ever written and it is
> naive — 1 of 1 — and the block carries none today.
>
> **A restart was owed and taken.** The daemon was restarted at
> **2026-08-28 15:17:21** <!--check:deploy--> <!--check:daemon_start-->
> (PID 3801574 → 3830192), `/health` answers **200**
...
> The schema was checked at head *before* the signal, `SNAG-DB-005`'s
> rule.
>
> **Alembic head is 017**<!--check:migration_head--> and the live schema
> matches it. Unmoved — this sitting wrote code and no migration.
>
> **The suite is 2792**, from 2795: **12 added and 15 retired**, and the
> total going *down* is the whole reason the sum is written. The twelve
> are `tests/test_ops_claims.py`'s, of which nine are the retired
> check's three-zone drive re-homed as `TestTheInstantCarriesItsZone`;
> the fifteen left with `expiry_naive_instant`, since every member of
> `CHECKS` names an *open* entry. Seven mutations driven, each red on
> exactly the right test — and rendering the pin in the marker's own
> zone breaks the founding case at `Europe/London` and
> `America/New_York` and **passes at UTC**, where the two renderings are
> the same string, so the parametrised test names which of its rows are
> witnesses and which is the control.
>
> **The check reported `mismatch` against the fix that closed its
> entry**, naming both halves of the shape-of-fix the entry wrote down —
> the offset-bearing instant parsing, and the zoneless one no longer
> being read as a moment. That is the entry refuted by the instrument
> built to watch it rather than by its author's say-so, which is the
> opposite of Session 111's retired check and worth the contrast.
>
> **The entry counts were re-derived, not carried**: **99 entries either
> side, open 20 → 19**, through `estate.snags.read_snags`. Every open
> entry still names a check (19 of 19) and `sysadmin-check-snags` exits
> **0**.
