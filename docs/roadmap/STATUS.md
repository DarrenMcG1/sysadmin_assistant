...
> **2026-09-14 18:19:50** <!--check:deploy--> <!--check:daemon_start-->,
> `NRestarts=12`, `/health` 200; the budget was checked first and the
...
> packaged head <!--check:schema-->, and `alerts` holds **7** unresolved
> rows <!--check:alerts-->,
> `High disk usage on /`,
> `Project ImbaBots next action idle`,
> `Project Athenaeum next action idle`,
> `Project alfred-glance next action idle`,
> `Project InvestingAssistant next action idle`,
> `Estate port 3400 registry breach` and
> `Estate port 3401 registry breach`, **7**
...
> *(The seventh opened at 08:20:52 on 2026-09-21, **during** the sitting
> that named it, and it is a live side-by-side specimen of
> `SNAG-ESTATE-009` rather than news about the box. Port 3401 is an Astro
> dev server in `app-code-oss-5162.scope`, the same shape as 3400 above
> it; 3400 is quietened to `info` because the six-hourly sweep had seen
> it, and 3401 speaks at `warning` because it started inside a sweep
> window. Two adjacent ports, one holder shape, two rungs, differing only
> by sweep age — which is that entry's whole claim, and it has never had
> a paired specimen before. It is named in the **stable** set on one
> episode, which is a statement about evidence rather than about
> behaviour: 1 row, first seen today, never resolved. `3110` and `8110`
> below are the same family declared flapping on 3 episodes each, so the
> day this one resolves and re-raises it belongs there instead.)*
