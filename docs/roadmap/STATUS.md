...
>
> **18 open, 0 unchecked** *(2026-08-31, Session 142: 20 → 18 as
> `SNAG-SCHED-001` and `SNAG-SCHED-003` closed together, and both
> their checks retired with them — a check naming a closed entry is
> refused by the sweep that already guards `CHECKS`, so the count
