...
> **The understudy remembers, and the reminder it could not reach was a
> restart-cadence problem nobody had measured.** `SNAG-TRAY-008` is
> **FIXED**, both faces. `DesktopNotifier._spoken` was in memory and the
> reminder sweep's population was exactly its keys, so a 24-hour
> reminder needed a 24-hour process — and `sysadmin.service` started
> **111 times in 28.26 days** at a median uptime of **1.77 h**, with
> **5 of 110** lives reaching 24 hours. `SNAG-TRAY-007`'s fix was
> structurally unavailable on **95 %** of this daemon's lives.
>
> **The entry's own shape-of-fix is unreachable as written, and the same
> measurement is why.** It asks for adoption *"only when the tray has
> been absent for a full `reminder_hours`"*; `TrayPresence` is in-memory
> and monotonic by deliberate design, so a process observes 24 h of
> absence only by living 24 h. Shipped as a **refusal** the fix would
> have been correct, green and inert. It ships as an **anchor** — the
> absence sets the adopted fault's clock *back*, capped at one interval
> — which keeps the quiet-by-construction property the entry asked for
> and is reachable on a 1.77-hour daemon.
>
> **The two faces are multiplicative, not independent** — `SNAG-AGENT-008`'s
> shape. Adoption alone re-adopts on every restart and re-arms its own
> anchor, so it would never speak; the store alone leaves face 1 exactly
> as filed. A fix for one half is not half the benefit, it is none.
>
> **The clock became a wall clock**, which is a change of reading and
> not only of storage: no monotonic value survives a process, and on
> Linux `CLOCK_MONOTONIC` does not survive a **suspend** either, so a
> workstation asleep overnight paid nothing towards an interval that is
> precisely about elapsed human time. `TrayPresence` keeps monotonic for
> its own 180-second question. The two now differ on purpose.
>
> **A test asserted the defect as correct behaviour and had to be
> inverted.** `test_a_fault_the_tray_announced_is_never_adopted` had
> been green since Session 55. Twenty-eight mutations were driven and
> every one lands red on a test about its own rule; **six passed against
> deliberately broken code first**, four of them upsert columns no fake
> can witness, because a fake replaces the whole row on conflict and so
> agrees with an `ON CONFLICT` that keeps the old value.
>
> **The probe harness leaked three rows into the live database before it
> was hardened**, which is not hypothetical: `_remember` must commit, and
> `rolled_back_drive` rolled back a plain session. Three rows into
> `alerts` and three into `desktop_notifications`, counted either side
> and deleted by hand. The session now joins the connection's
> transaction by savepoint. A harness that cannot survive the code it
> drives is a control the next fix breaks.
>
> **Deploying it found a second, independent reason the reminder path was
> inert, and no amount of reading would have.** `DesktopNotifier`
> resolved `get_session_factory()` — the *application's* pooled engine —
> while every call it makes runs on a loop that is not the
> application's. The first query out of a restarted daemon raised
> `RuntimeError: got Future … attached to a different loop`.
> **`_still_open` has carried that defect since Session 55 and never
> once executed on this box**, because the sweep's old first gate
> returned before reaching it — so `SNAG-TRAY-007`'s reminder could not
> have worked here even for a fault the daemon *had* announced, and this
> entry's own symptom is what hid it. `_factory()` returns
> `get_scheduler_session` now. Found by restarting and reading three
> `Log error: sysadmin.service` rows out of the live table.
>
...
> have restated it.
>
> **A restart was owed and taken — three times, the second to carry the
> loop fix above.** The daemon was last restarted at
> **2026-08-28 16:38:05** <!--check:deploy--> <!--check:daemon_start-->
> (PID 3865932 → 3875592; 3859841 → 3865932 at 16:18:54, and 3830192 →
...
>
> **The suite is 2801**, from 2792: **37 added and 28 retired**. The
> twenty-eight left with `understudy_forgets`, since every member of
> `CHECKS` names an *open* entry; the thirty-seven are 30 in
> `tests/test_desktop_notifier.py` — two of them guarding the
> scheduler-safe session, one behavioural and one an AST sweep refusing
> the pooled name — and 7 in the new
> `tests/test_desktop_store_live.py`, which is the retired check's
> two-sweep timeline re-homed against the real database —
> `FROZEN_TABLES`' rule, and it is **stronger than the check it
> replaces**: once adoption landed, "the restarted instance restated its
> predecessor's fault" was producible by adoption alone, so the live
> test asks *how* it inherited it.
>
> **The check reported `mismatch` against the fix that closed its
> entry**, both faces, with **0 rows of residue**.
>
> **The entry counts were re-derived, not carried**: **99 entries either
> side, open 19 → 18**, through `estate.snags.read_snags`.
