...
> **The largest population was in the family the entry never named, and
> fixing it half-closed a different entry.**
> `SNAG-AGENT-009` is **FIXED**, in the order Session 109's measurement
> ranked: `SysAdminAgent._raise_judged` (**838** held events),
> `EstateJudgeAgent._execute` (**131**), `_maintain_port_alerts` (**0 in
> 65 runs**, and its docstring now says so rather than leaving the
> silence to be read as an oversight). A held judgement now rewrites the
> standing row's `message` **and** its `details`, and only when the
> recomputed text differs.
>
> **`BaseAgent.refresh_alert` owns the comparison and the write; finding
> the row stays with each caller.** The three reach it three different
> ways for reasons of their own — the estate judge already holds the ORM
> rows, the port family bounds its read by a title prefix, and the
> sysadmin agent keeps the title-only snapshot `SNAG-AGENT-007` gave it
> — so a base class taking a *title* would own a predicate its
> subclasses state three ways.
>
> **The 838 needed a read the snapshot deliberately cannot supply, and
> lazily is the right answer rather than the cheap one.** Widening
> `_open_alert_titles` to carry `message` and `details` is bounded by
> *the table*, and this agent's families reached 51,924 open rows before
> `SNAG-AGENT-004`; a read at the hold is bounded by *the judgements the
> run made*. Live at **665,937 rows: 84 buffers, 0.098 ms** — and on
> `idx_alerts_active`, not the agent-scoped index the first draft of the
> docstring claimed. One open row makes either partial index free and
> the planner takes the older one, which is exactly what
> `SNAG-AGENT-007` recorded about its own pair. Reading the code would
> have shipped the wrong index name; `EXPLAIN` corrected it.
>
> **Three rules the build settled that the entry did not name.**
> `details` is compared through a **JSON round trip**, because `JSONB`
> has no tuple and a plain `!=` would report a difference no write can
> settle — the gate switched off by a type, with the counter reporting
> corrections that corrected nothing. What is *stored* is still the
> caller's dict, so a raise and a refresh handed one input write one
> row. And `_written_titles` is a **second set**: `_open_titles` gained
> the titles this run raised so two identically-named GPUs dedup, and
> refreshing on that branch would let the *last* of two same-titled
> judgements overwrite the first, which is a question the run must not
> answer twice.
>
> **The gate is a floor, not a promise of quiet.** For a family whose
...
> poll writes. Bounded anyway at the held rate, **~0.26 per run**, and
> it is an `UPDATE` to a row that already exists: `SNAG-AGENT-006`'s
> objection was about `INSERT`s accumulating and does not transfer.
>
> **`SNAG-ESTATE-010` is half-closed, and its own check said so out
> loud.** That entry claims a quieter judgement reaches neither the
> severity nor the `holder` blob of a standing row. The blob now
> arrives; the rung still does not, by Session 39's design. Its check
> flipped to `mismatch` on the first run after the fix, and the clause
> had to come **out** of `QuietenReading.reached` rather than the
> verdict being accepted — a `reached` still reading the blob answers
> `mismatch` whatever happens to the rung, which is a control this fix
> broke. The falsification asserting the blob alone was a mismatch is
> kept and **inverted**, so the day it stops arriving this says so.
>
> **Verified live rather than only against fixtures.** Through the real
> `_raise_judged` against the real database in a rolled-back
...
> `refreshed=0`, and **0 rows of residue**.
>
> **A restart was owed and taken.** The daemon was restarted at
> **2026-08-28 13:03:15** <!--check:deploy--> <!--check:daemon_start-->,
> `/health` answers **200** <!--check:health-->, `alembic current` reads
> 017 at the packaged head <!--check:schema-->, and `alerts` holds **1**
> unresolved row <!--check:alerts--> — still `info: Weekly disk review
> ready` <!--check:open_titles-->, which is `SNAG-AGENT-010` and is
> untouched by this sitting: that row was never *held*, so this fix
> cannot reach it.
>
> **Alembic head is 017**<!--check:migration_head--> and the live schema
> matches it. Unmoved — this sitting wrote code and no migration.
>
> **The suite is 2764**, from 2750: **14 added** and none removed.
> `tests/test_alert_refresh.py` is 9 of them and the three callers took
> the other 5. All five falsifications fire — the refresh made a no-op
> (7 tests), the gate removed (7), the JSON round trip removed (1),
> `details` left frozen (7), the two sets merged (1).
>
> **The stand-ins were what had to change, in four files**, which is the
> part worth carrying: `test_unit_ports.py` answered the dedup read with
> *titles*, `test_estate_judge_agent.py`'s `FakeAlert` had no `message`,
> `test_alert_dedup.py`'s fake ignored the WHERE clause and could not
> answer `.first()`, and `conftest.mock_session`'s bare `AsyncMock`
> returned a coroutine from `.scalars()` — failing in a way no database
> does. `SNAG-ESTATE-010`'s probe found its standing row by
> `message == PROBE_MESSAGE`, the value this fix rewrites, and now keys
> on the title.
>
> **The entry counts were re-derived, not carried** (estate-manager's
> message `8c1706d3`): **99 entries either side, open 24 → 23**, through
> `estate.snags.read_snags` at their committed `047eb8a` on a clean
> tree.
