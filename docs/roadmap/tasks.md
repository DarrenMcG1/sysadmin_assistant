...
- [x] **This repository has published 67 entries for at least four sittings against a live 69.** The reader became `estate.snags` in estate-lib and now counts the two `### Session NN write-up` headings under `## Fixed Issues`. Both are `is_open: False`, which is why the open count never moved and why the drift stayed invisible — the figure the board publishes was right throughout. Two test docstrings corrected
