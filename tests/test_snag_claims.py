...
class TestTheReviewSlotCollisionCheck:
    ...
    *still holds* over the other, which is
    ...
    # -- SNAG-SCHED-002: the deferring gate is evidence, never a refutation --

    def test_the_live_deferring_gate_is_reported(self):
        """The defect this class shipped with, pinned at the live binding.

        Until 2026-08-30 the detail read ``gpu gates under sysadmin/:
        none`` while :mod:`sysadmin.core.llm_client` called
        ``ensure_gpu_idle`` on every generation — a constant observation
        from a four-name vocabulary nothing in the population could have
        moved."""
        detail = "\n".join(check_review_slot_collides().detail)
        assert "deferring gates under sysadmin/: sysadmin/core/llm_client.py" in detail
        assert "deferring gates under sysadmin/: none" not in detail

    def test_the_deferring_report_has_a_witness(self, tmp_path):
        """...and the line above is not green because the walk is blind.

        A walker that had stopped reading identifiers reports the live
        binding exactly as a healthy one fails to, so the discriminator
        is a file that binds *nothing* and must come back ``none``."""
        clean = tmp_path / "review.py"
        clean.write_text("def f():\n    return 1\n", encoding="utf-8")
        with patch.object(snag_claims, "_python_files", lambda roots: [clean]):
            detail = "\n".join(check_review_slot_collides().detail)
        assert "deferring gates under sysadmin/: none" in detail

    def test_the_premise_is_read_from_the_producer(self):
        """The binding is asserted at its owner rather than remembered.

        If ``llm_client`` stops gating, this test names that rather than
        the report going quietly empty — the failure mode of a deleted
        gate is silence, which is what `SNAG-SCHED-002` was."""
        source = (
            Path(__file__).parent.parent / "sysadmin" / "core" / "llm_client.py"
        ).read_text(encoding="utf-8")
        assert "ensure_gpu_idle(" in source
        assert "except GpuBusy" in source

    def test_a_deferring_gate_does_not_refute_the_entry(self, tmp_path):
        """The repair that was *not* made, and the reason it was not.

        Adding ``ensure_gpu_idle`` to the refuting set is the obvious fix
        and retires ``SNAG-SCHED-001`` on the strength of a gate that
        predates it by eighteen days — the gate is *why* the collision
        costs a digest rather than a slow review."""
        deferring = tmp_path / "review.py"
        deferring.write_text(
            "from estate.gpu import GpuBusy, ensure_gpu_idle\n"
            "def f():\n"
            "    try:\n"
            "        ensure_gpu_idle('0000:03:00.0', 25)\n"
            "    except GpuBusy:\n"
            "        return None\n",
            encoding="utf-8",
        )
        with patch.object(snag_claims, "_python_files", lambda roots: [deferring]):
            measurement = check_review_slot_collides()
        assert measurement.verdict == "match"
        assert "deferring gates under sysadmin/: none" not in "\n".join(measurement.detail)

    def test_arbitration_still_refutes_with_a_deferring_gate_beside_it(self, tmp_path):
        """The two limbs are independent, so the deferral must not mask
        the arbitration the check exists to spot."""
        both = tmp_path / "review.py"
        both.write_text(
            "from estate.gpu import ensure_gpu_idle\n"
            "from estate import estate_queue\n",
            encoding="utf-8",
        )
        with patch.object(snag_claims, "_python_files", lambda roots: [both]):
            measurement = check_review_slot_collides()
        assert measurement.verdict == "mismatch"
        assert "first named fix" in measurement.note

    def test_the_two_vocabularies_are_disjoint(self):
        """A name in both sets would make the split decide nothing, which
        is the whole content of the fix."""
        assert not (snag_claims.GPU_ARBITRATION_NAMES & snag_claims.GPU_DEFERRAL_NAMES)
        assert snag_claims.GPU_DEFERRAL_NAMES
        assert snag_claims.GPU_ARBITRATION_NAMES
...
class TestTheReviewChainDrainCheck:
    """``review_chain_inside_drain`` — the whole chain, not one slot.

    ``SNAG-SCHED-001`` watches the disk review against the estate's
    granted band.  This watches all three against the occupancy that band
    is the *tail* of, which is the finding that entry's own measurement
    turned up: 05:00 and 05:15 are worse off than 05:45 and no schedule
    leaf in that entry reaches them.
    """

    HEALTH, LOG, DISK = 5 * 3600, 5 * 3600 + 900, 5 * 3600 + 2700

    @classmethod
    def _payload(cls, health, log, disk, n=6):
        """A snapshot payload for the three slots.

        Each argument is either one busy percent repeated ``n`` times, or
        an explicit list — **which the majority rule needs**, since a
        uniform slot is either 0/n or n/n and those are the two readings
        a majority rule and a unanimity rule agree about.  A specimen that
        cannot separate them is not a specimen for this rule.
        """
        rows = []
        for at, busy in ((cls.HEALTH, health), (cls.LOG, log), (cls.DISK, disk)):
            samples = list(busy) if isinstance(busy, (list, tuple)) else [busy] * n
            rows.extend(
                {
                    "at": at,
                    "gpu": {
                        "card0": {"vram_total_mb": 24560, "gpu_percent": sample},
                        "card1": {"vram_total_mb": 2048, "gpu_percent": 0},
                    },
                }
    ...
    def test_it_holds_on_the_box_as_measured(self):
        """The live half. 05:00 and 05:15 read wholly inside the drain;
        05:45 sits at its edge, which is why the entry that watches 05:45
        alone under-reads the fault."""
        measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "match"
        detail = "\n".join(measurement.detail)
        assert "generating into a held card: health_review, log_review" in detail

    def test_the_live_reading_has_a_witness(self):
        """...and it is not constant: the disk slot reads *below* the
        majority off the same query, so the three populations are
        discriminated rather than uniformly loud."""
        detail = "\n".join(check_review_chain_inside_drain().detail)
        assert "disk_review 5/12 busy" in detail or "disk_review" in detail
        busy = {
            part.split()[0]: part.split()[1]
            for part in detail.split("\n")[1].split("; ")
        }
        assert busy["health_review"] != busy["disk_review"]

    def test_an_arbitrating_gate_refutes_it(self, tmp_path):
        gated = tmp_path / "review.py"
        gated.write_text("from estate import estate_queue\n", encoding="utf-8")
        with patch.object(snag_claims, "_python_files", lambda roots: [gated]):
            measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "mismatch"
        assert "durable fix" in measurement.note

    def test_slots_clear_of_the_occupancy_refute_it(self):
        with patch.object(
            snag_claims, "query_one", lambda _s: (self._payload(0, 1, 2), "")
        ):
            measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "mismatch"
        assert "without anything here having been fixed" in measurement.note

    def test_one_slot_inside_is_enough_to_hold(self):
        """The claim is about the chain, so a partial move does not
        refute it — which is the argument against moving leaves one at a
        time.

        The surviving slot is **4 of 6**, deliberately not 6 of 6: a
        unanimous slot satisfies a majority rule and a unanimity rule
        alike, so the first draft of this test passed against a mutation
        that had replaced the majority with unanimity.  A specimen that
        both rules agree about witnesses neither."""
        with patch.object(
            snag_claims,
            "query_one",
            lambda _s: (self._payload(0, 0, [99, 99, 99, 99, 0, 0]), ""),
        ):
            assert check_review_chain_inside_drain().verdict == "match"

    def test_a_slot_at_exactly_half_is_not_inside(self):
        """The rule is a strict majority, and the boundary is the only
        place a majority rule and a "some samples were busy" rule
        differ."""
        with patch.object(
            snag_claims,
            "query_one",
            lambda _s: (self._payload(0, 0, [99, 99, 99, 0, 0, 0]), ""),
        ):
            assert check_review_chain_inside_drain().verdict == "mismatch"

    def test_a_thin_slot_is_unknown_not_clean(self):
        """``ports_checked``'s rule: one poll on one night is not an
        occupancy, and zero-because-blind must not read as
        zero-because-clean."""
        with patch.object(
            snag_claims, "query_one", lambda _s: (self._payload(99, 99, 99, n=2), "")
        ):
            measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "unknown"
        assert "not an occupancy" in measurement.note

    def test_it_is_unknown_when_the_drain_timer_is_gone(self):
        """The premise is another repository's unit; its absence is the
        premise dying rather than the claim being refuted."""
        with patch.object(snag_claims, "DRAIN_UNIT", "sysadmin-no-such.timer"):
            measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "unknown"
        assert "premise dying" in measurement.note
    ...
    def test_it_is_unknown_when_the_database_will_not_answer(self):
        with patch.object(
            snag_claims, "query_one", lambda _s: (None, "postgres is down")
        ):
            measurement = check_review_chain_inside_drain()
        assert measurement.verdict == "unknown"
        assert "postgres is down" in measurement.note

    def test_it_is_unknown_when_the_payload_will_not_parse(self):
        with patch.object(snag_claims, "query_one", lambda _s: ("not json", "")):
            assert check_review_chain_inside_drain().verdict == "unknown"

    def test_the_card_is_chosen_by_vram_not_by_key(self):
        """The trap this would otherwise walk into: the collector writes
        ``card0``/``card1`` in enumeration order and records no PCI slot,
        so keying on ``card0`` reads the *iGPU* the day that order flips
        — and the iGPU is busy with desktop compositing, a different fact
        wearing the same number."""
        dgpu_first = {
            "card0": {"vram_total_mb": 24560, "gpu_percent": 7},
            "card1": {"vram_total_mb": 2048, "gpu_percent": 96},
        }
        flipped = {
            "card0": {"vram_total_mb": 2048, "gpu_percent": 96},
            "card1": {"vram_total_mb": 24560, "gpu_percent": 7},
        }
        assert snag_claims._dgpu_percent(dgpu_first) == 7
        assert snag_claims._dgpu_percent(flipped) == 7

    def test_an_unreadable_blob_yields_no_sample(self):
        assert snag_claims._dgpu_percent({}) is None
        assert snag_claims._dgpu_percent("not a dict") is None
        assert snag_claims._dgpu_percent({"card0": {"gpu_percent": 9}}) is None

    def test_the_threshold_is_the_gates_own(self):
        """Derived, never restated — a config edit that widens what the
        daemon tolerates must move this reading with it.

        **Asserted as provenance rather than as a value**, which is the
        trap this file has been caught by before: comparing the detail
        against ``get_config()`` passes just as well over a hard-coded
        ``25`` while the shipped threshold happens to be 25.  So the
        threshold is *moved* and the reading has to move with it — at
        100 % nothing is over the bar, and every slot must go quiet."""
        real = snag_claims.get_config()
        raised = SimpleNamespace(
            llm=SimpleNamespace(gpu_busy_threshold=100),
            schedules=real.schedules,
            database=real.database,
        )
        with patch.object(snag_claims, "get_config", lambda: raised):
            measurement = check_review_chain_inside_drain()
        detail = "\n".join(measurement.detail)
        assert "threshold of 100%" in detail
        assert "generating into a held card: none" in detail
        assert measurement.verdict == "mismatch"
