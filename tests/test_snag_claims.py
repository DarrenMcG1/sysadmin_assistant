...
# ---------------------------------------------------------------------------
# The twelfth check — SNAG-DOCS-005
# ---------------------------------------------------------------------------


def _stripping_reader(pattern):
    """``ops_claims.read_markers`` with ``pattern`` stripped first.

    The fix the entry proposes, in the two shapes anybody would write it.
    Patched over the real function rather than reimplemented around it, so
    the rest of the path under test — ``flatten``, ``MARKER_RE``,
    ``check_markers`` — stays the production one.
    """
    import sysadmin.ops_claims as ops

    def read_markers(region):
        flat = pattern.sub(" ", ops.flatten(region))
        return [ops.Marker(m.group(1), m.group(2).strip()) for m in ops.MARKER_RE.finditer(flat)]

    return read_markers


SAME_LENGTH_FIX = re.compile(r"(`+)[\s\S]*?\1")
NAIVE_FIX = re.compile(r"`[^`]+`")


class TestTheQuotedMarkerCheck:
    """``SNAG-DOCS-005`` — the sibling reads a quoted marker as a real one.

    Every test here is falsified against the behaviour it replaces, by
    patching the fix *in* rather than by asserting on a literal: the
    check's whole job is to notice a three-line edit landing in another
    module, and a test built from fixtures could not tell that edit from a
    rename.
    """

    def test_the_claim_still_holds_against_the_real_reader(self):
        found = snag_claims.check_quoted_marker_reads_as_real()
        assert found.verdict == "match"
        assert not found.note, "an unqualified match is the entry as filed"

    def test_all_three_shapes_leak_today(self):
        """The three observations, read off the evidence rather than the verdict."""
        found = snag_claims.check_quoted_marker_reads_as_real()
        probes = [line for line in found.detail if line.startswith("probe:")]
        assert len(probes) == 3
        assert all(line.endswith("True") for line in probes), probes

    def test_the_same_length_fix_refutes_the_claim(self):
        """The falsification, and the only verdict that is a closure."""
        with patch("sysadmin.ops_claims.read_markers", _stripping_reader(SAME_LENGTH_FIX)):
            found = snag_claims.check_quoted_marker_reads_as_real()
        assert found.verdict == "mismatch"
        assert "candidate for closure" in found.note

    def test_a_naive_fix_leaves_the_doubled_fence_and_is_not_a_closure(self):
        """Why the third probe exists, and it is measured rather than argued.

        Both obvious three-line fixes pass the single fence.  Only the
        same-length pattern survives ``` ``the `<!--check:x-->` marker`` ``` —
        the shape ``SNAG-DOCS-005``'s own body carries — so a check
        testing one fence would report ``` `[^`]+` ``` a clean closure.
        """
        with patch("sysadmin.ops_claims.read_markers", _stripping_reader(NAIVE_FIX)):
            found = snag_claims.check_quoted_marker_reads_as_real()
        assert found.verdict == "match", "the defect is not gone, so the entry is not refuted"
        assert "doubled fence still leaks" in found.note
        assert found.detail[2].endswith("True")
        assert found.detail[0].endswith("False") and found.detail[1].endswith("False")

    def test_the_control_region_reports_the_figure_as_unclaimed(self):
        """A finding's absence is only evidence once its presence is observed.

        Direction two is read off a *missing* ``unclaimed`` finding, which
        would also be missing if the pattern stopped matching, if the
        region stopped parsing, or if the convention were retired.  The
        control is what tells those apart.
        """
        markers, findings = snag_claims.ops_probe("")
        assert markers == frozenset()
        assert f"unclaimed:{snag_claims.OPS_PROBE_CLAIMED}" in findings

    def test_a_quotation_registers_as_a_marker(self):
        """The mechanism itself, one layer under the verdict."""
        quotation = snag_claims.OPS_PROBE_SINGLE_FENCE.format(
            claimed=snag_claims.OPS_PROBE_CLAIMED, unknown=snag_claims.OPS_PROBE_UNKNOWN
        )
        markers, _ = snag_claims.ops_probe(quotation)
        assert markers == {snag_claims.OPS_PROBE_CLAIMED, snag_claims.OPS_PROBE_UNKNOWN}

    def test_a_probe_key_that_became_a_real_check_is_unknown(self):
        """The probe stating it can no longer isolate the question."""
        with patch.object(
            snag_claims, "OPS_PROBE_UNKNOWN", snag_claims.OPS_PROBE_CLAIMED
        ):
            found = snag_claims.check_quoted_marker_reads_as_real()
        assert found.verdict == "unknown"
        assert "no longer isolates the question" in found.note

    def test_a_probe_key_without_a_claim_pattern_is_unknown(self):
        with patch.object(snag_claims, "OPS_PROBE_CLAIMED", "not_a_claim_key"):
            found = snag_claims.check_quoted_marker_reads_as_real()
        assert found.verdict == "unknown"
        assert "claim pattern" in found.note


class TestTheMarkerSurvey:
    """The live margin, which is evidence and never a verdict — rule 1.

    The entry says its population is empty because the block is written to
    be read.  It is empty because of where a ``## `` heading falls, and
    this is what measures the difference.
    """

    REGION = (
        "# Probe\n\n## Quick Status\n\n> The application serves **7 routes**.\n\n"
        "## Recently Completed\n\nThe entry's body carries `<!--check:routes-->`.\n"
    )

    def test_a_quoted_marker_outside_the_region_is_located(self, tmp_path):
        path = tmp_path / "STATUS.md"
        path.write_text(self.REGION, encoding="utf-8")
        survey, problem = snag_claims.survey_quoted_markers(path)
        assert not problem
        assert survey.in_region == 0
        assert [(m.key, m.distance, m.implemented) for m in survey.outside] == [
            ("routes", 3, True)
        ]

    def test_a_marker_inside_the_region_is_not_counted_as_outside(self, tmp_path):
        """The falsification: identical text, the heading moved below it."""
        path = tmp_path / "STATUS.md"
        path.write_text(
            self.REGION.replace("## Recently Completed\n\n", ""), encoding="utf-8"
        )
        survey, _ = snag_claims.survey_quoted_markers(path)
        assert survey.in_region == 1
        assert survey.quoted_in_region == 1
        assert survey.outside == ()

    def test_an_unreadable_document_is_a_sentence_and_never_a_crash(self):
        survey, problem = snag_claims.survey_quoted_markers(Path("/nonexistent/STATUS.md"))
        assert survey is None
        assert "could not be read" in problem

    def test_a_document_with_no_quick_status_is_a_sentence(self, tmp_path):
        path = tmp_path / "STATUS.md"
        path.write_text("# No block here\n", encoding="utf-8")
        survey, problem = snag_claims.survey_quoted_markers(path)
        assert survey is None
        assert "Quick Status" in problem

    def test_the_quoted_difference_is_a_multiset(self, tmp_path):
        """One key written twice, once quoted and once bare, is one quotation.

        A set difference reads that line as carrying no quotation at all,
        which is the shape a sentence explaining its own marker takes.
        """
        assert snag_claims._quoted_only(["routes", "routes"], ["routes"]) == ["routes"]
        assert snag_claims._quoted_only(["routes"], ["routes"]) == []

    def test_the_live_region_carries_no_quoted_marker(self):
        """The entry's own measurement, re-measured against the real file.

        Asserted as the *mechanism reaching the real document* rather than
        as a number: a region that suddenly carried one would mean the
        hazard had acquired a live member, which is a finding for the
        sitting rather than a red test.

        **Its blind spot is stated rather than implied.**  A stripper that
        stopped stripping would report ``quoted_in_region == 0`` too, so
        this one cannot tell health from a broken instrument — driven and
        confirmed, it stays green against exactly that edit.
        :meth:`test_a_marker_inside_the_region_is_not_counted_as_outside`
        is the falsifiable half, and it is built rather than live for that
        reason.
        """
        survey, problem = snag_claims.survey_quoted_markers()
        assert not problem
        assert survey.in_region >= 1, "the convention is in use, or this measures nothing"
        assert survey.quoted_in_region == 0
