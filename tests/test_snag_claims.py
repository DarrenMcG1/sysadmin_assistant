...
# ---------------------------------------------------------------------------
# The registered checks, each falsified against the behaviour it detects
# ---------------------------------------------------------------------------


class TestTheStandInDisambiguator:
    """``_marker`` is what a falsification's hypothetical fix is built from.

    ``SNAG-TEST-001``: three guards rested on ``hash(s) % 997``, whose
    value is seeded per process, so they were red about once in 997 runs
    and said nothing about the check on the other 996.  A guard that is
    wrong at a rate nobody has measured is worth less than its green
    runs read.

    **Those three guards retired with ``SNAG-LOG-013`` on 2026-09-03 and
    this class did not** — ``FROZEN_TABLES``' rule, and the AST sweep
    below is the half that matters: it refuses the builtin by name and
    names ``_marker`` as what to reach for instead, so the helper has to
    exist for the refusal to be actionable.  Deleting a guard with its
    last user takes the guard against the defect coming back at the
    moment nothing is exercising it.
    """
...
class TestVacuousGuardUngated:
    """``SNAG-TEST-006`` — nothing on the close path runs the suite under coverage.

    Driven at seven stand-ins, because five of the seven readings are the
    ones that would let something pass unnoticed: a close path that has
    been fixed must read ``mismatch`` whether the coverage token sits in
    a root or in a script a root invokes, a close path this sweep cannot
    parse must read ``unknown``, and the two shapes that merely *name*
    coverage — a comment and an ``echo`` — must not refute an entry that
    is precisely about a measure being documented and not wired.
    """

    KEY = "vacuous_guard_ungated"

    #: A close-path root that is not being tested has to carry a guard,
    #: or every stand-in falls into the ``unknown`` branch and the test
    #: passes for a reason that has nothing to do with what it asserts.
    GUARDED = "./scripts/check-migrations.sh --quiet\n"

    @staticmethod
    def _close_path(tmp_path, monkeypatch, postflight: str, precommit: str = GUARDED):
        post = tmp_path / "claude-postflight.sh"
        pre = tmp_path / "claude-precommit.sh"
        post.write_text(postflight, encoding="utf-8")
        pre.write_text(precommit, encoding="utf-8")
        monkeypatch.setattr(snag_claims, "CLOSE_PATH_SCRIPTS", (post, pre))
        return tmp_path

    def test_the_live_close_path_still_carries_the_gap(self):
        """The entry as filed: four guards wired, coverage never invoked."""
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "match"
        assert "coverage invocations: none" in measured.detail

    def test_a_close_path_that_runs_coverage_refutes_it(self, tmp_path, monkeypatch):
        """The stand-in modelling the **fix**.

        Without it the check is coupled to the unfixed behaviour and a
        landed gate reads as a broken probe rather than as a closure.
        """
        self._close_path(
            tmp_path,
            monkeypatch,
            self.GUARDED + "uv run --with coverage coverage run -m pytest -q\n",
        )
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "mismatch"
        assert "coverage" in measured.note

    def test_a_gate_wired_as_its_own_script_refutes_it(self, tmp_path, monkeypatch):
        """The one-hop half, and the only test the hop is needed for.

        The fix this entry describes is a *new script* wired at
        ``claude-postflight.sh``.  Its name carries no coverage token, so
        a sweep of the roots alone reports ``match`` over exactly the
        thing it is watching for — driven by removing the hop, which
        turns this test and no other red.
        """
        (tmp_path / "check-vacuous-guards.sh").write_text(
            "uv run --with coverage coverage run -m pytest -q\n", encoding="utf-8"
        )
        self._close_path(
            tmp_path, monkeypatch, self.GUARDED + "./scripts/check-vacuous-guards.sh\n"
        )
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "mismatch"
        assert "check-vacuous-guards.sh" in measured.note

    def test_a_gate_wired_at_the_commit_rather_than_the_close_refutes_it(
        self, tmp_path, monkeypatch
    ):
        """Both roots are read, because a gate at either closes the entry.

        The entry names ``claude-postflight.sh`` as where its fix would
        be wired, which is a reason to read that file and not a reason to
        read only it — a sweep of the close alone would report ``match``
        over a gate wired one script over.
        """
        self._close_path(
            tmp_path,
            monkeypatch,
            self.GUARDED,
            precommit=self.GUARDED + "uv run coverage run -m pytest -q\n",
        )
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "mismatch"
        assert "claude-precommit.sh" in measured.note

    def test_the_pytest_cov_spelling_also_refutes_it(self, tmp_path, monkeypatch):
        """A gate need not use ``coverage`` the command to close this."""
        self._close_path(tmp_path, monkeypatch, self.GUARDED + "uv run pytest --cov=tests -q\n")
        assert snag_claims.check_vacuous_guard_ungated().verdict == "mismatch"

    def test_an_echo_naming_coverage_does_not_refute_it(self, tmp_path, monkeypatch):
        """A mention is prose printed at the operator, never wiring.

        The live witness is in the file: ``claude-postflight.sh`` prints
        ``Consider running your test suite`` and runs none, and both roots
        print the name of a guard on an ``echo``.  The advisory line is
        this script's own idiom, so it is how a sitting would most likely
        add coverage *as advice* — and a check refuted by advice would
        report the gate wired on the day somebody suggested it.
        """
        self._close_path(
            tmp_path,
            monkeypatch,
            self.GUARDED + 'echo -e "  Consider: uv run coverage run -m pytest"\n',
        )
        assert snag_claims.check_vacuous_guard_ungated().verdict == "match"

    def test_a_commented_out_coverage_run_does_not_refute_it(self, tmp_path, monkeypatch):
        """``check_handoff_shape_unguarded``'s rule, one entry over.

        A check that read prose would report this entry closed by the
        paragraph in ``snag_list.md`` describing the fix.
        """
        self._close_path(
            tmp_path, monkeypatch, "# TODO: coverage run -m pytest here\n" + self.GUARDED
        )
        assert snag_claims.check_vacuous_guard_ungated().verdict == "match"

    def test_a_close_path_naming_no_guard_is_unknown_rather_than_match(
        self, tmp_path, monkeypatch
    ):
        """`ports_checked`'s rule: zero-because-blind is not zero-because-clean.

        The live close path gates on four named scripts, so finding none
        means this sweep has stopped reading the files it thinks it is
        reading — and ``match`` there is a confident statement about a
        close path nobody looked at.
        """
        self._close_path(tmp_path, monkeypatch, "true\n", precommit="true\n")
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "unknown"
        assert "zero-because-blind" in measured.note

    def test_an_unreadable_root_is_unknown(self, tmp_path, monkeypatch):
        """Half a close path is not a close path.

        The absent root is *named*, because a sweep that quietly dropped
        it would go on reporting on the other one — which is the same
        collapse the guard witness above refuses, one file up.
        """
        post = tmp_path / "claude-postflight.sh"
        post.write_text(self.GUARDED, encoding="utf-8")
        monkeypatch.setattr(
            snag_claims, "CLOSE_PATH_SCRIPTS", (post, tmp_path / "absent.sh")
        )
        measured = snag_claims.check_vacuous_guard_ungated()
        assert measured.verdict == "unknown"
        assert "absent.sh" in measured.note

    def test_the_mismatch_note_names_the_half_a_gate_has_not_closed(
        self, tmp_path, monkeypatch
    ):
        """The third verdict, reported rather than asked for.

        This check reads whether a gate exists; it cannot read whether
        the gate carries the verdict the entry says it owes.  A check
        answering both would return one limb and stay silent about the
        other, so the reading is carried in the note — whoever lands the
        gate is told, at the moment they would otherwise close the entry,
        that line coverage cannot reach ``assert all(f(x) for x in
        live)``.
        """
        self._close_path(tmp_path, monkeypatch, self.GUARDED + "coverage run -m pytest\n")
        note = snag_claims.check_vacuous_guard_ungated().note
        assert "assert all(f(x) for x in live)" in note
        assert "two verdicts" in note

    def test_the_mention_rule_moves_no_verdict_today(self):
        """What the rule costs and what it buys, measured rather than assumed.

        It drops 132 lines from the two roots and **none of them names a
        coverage token**, so its population for the *answer* is empty and
        this records that rather than letting the rule imply it fires.
        What it does discriminate is live: two dropped lines name a guard
        the script does not run, and one is the advisory suite line the
        ``echo`` test above is built on.
        """
        dropped = [
            line
            for root in snag_claims.CLOSE_PATH_SCRIPTS
            for line in snag_claims._shell_executable_lines(root)
            if line not in snag_claims._shell_invocation_lines(root)
        ]
        assert dropped
        assert not [
            line for line in dropped for t in snag_claims.COVERAGE_INVOCATIONS if t in line
        ]
        assert [line for line in dropped if "test suite" in line]

    def test_the_close_path_is_both_scripts_and_the_commit_half_is_borrowed(self):
        """A statement test, because every drive above patches the constant.

        The stand-ins replace ``CLOSE_PATH_SCRIPTS`` wholesale, so no
        mutation *of* it can be observed behaviourally — narrowing it to
        the close root alone leaves all of them green.  What is asserted
        here is the constant itself: both roots, both on disk, and the
        commit half taken by identity from ``check_handoff_shape_unguarded``
        rather than spelled again, so one path cannot be two strings that
        disagree.

        **By identity, and the falsification is what settled that.**  A
        restated ``REPO_ROOT / "scripts" / "claude-precommit.sh"`` is
        ``==`` to the borrowed constant, because ``Path`` compares by
        value — so ``in`` asserts a value where this means provenance and
        goes green over the copy.  A fresh ``Path`` is a fresh object, so
        ``is`` discriminates and no AST walk is needed here.
        """
        assert any(p is snag_claims.PRECOMMIT_SCRIPT for p in snag_claims.CLOSE_PATH_SCRIPTS)
        assert [p.name for p in snag_claims.CLOSE_PATH_SCRIPTS] == [
            "claude-postflight.sh",
            "claude-precommit.sh",
        ]
        assert all(p.is_file() for p in snag_claims.CLOSE_PATH_SCRIPTS)

    def test_the_entry_names_this_check(self):
        """Rule 4: the marker names a check and the check names its entry."""
        entry = next(
            e for e in snag_claims.load_entries()[0] if e.snag_id == "SNAG-TEST-006"
        )
        assert self.KEY in entry.markers
        assert CHECKS[self.KEY].snag == entry.snag_id
