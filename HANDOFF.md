# Handoff — 2026-09-02 (Session 154)

## Next action

Fix `SNAG-SVC-004` — project the fold's step provenance into `health_review._service_facts` as its own field rather than by sending `detail` wholesale into a prompt that is figure-free by construction, which is `stands_for`'s treatment for a second fact, and that entry's own last bullet argues the check belongs with the sitting deciding the field's shape because it asserts the fix rather than the defect.
