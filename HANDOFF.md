# Handoff — 2026-09-03 (Session 159)

## Next action

Decide `SNAG-DB-006` by measurement rather than by preference — the seven stuck `running` rows in `agent_runs`, up from the five the entry counted on 2026-08-25 and dated 2026-08-14 to 2026-08-28, are the evidence for whether `cancelled` was meant for a run killed mid-`_execute`, and the entry's two candidate fixes are opposite, so date those rows against the daemon's restarts before either dropping the constraint value or writing the path that would fill it.
